...
class ZipTree:
	...
	def zip(self, x: Node):
		def zipFunction(leftNode: Node, rightNode: Node):
			if leftNode is None:return rightNode
			if rightNode is None: return leftNode
			if rightNode.rank > leftNode.rank:
				rightNode.left = zipFunction(leftNode, rightNode.left)
				return rightNode
			else:
				leftNode.right = zipFunction(leftNode.right, rightNode)
				return leftNode
		return zipFunction(x.left, x.right)
